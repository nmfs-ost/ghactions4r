...
test_that("use_spell_check() works", {
  ...
  # Test that the function creates the full spell check workflow file
  report_level <- c("warning", "error")
  for (level in report_level) {
    use_spell_check(use_full_spell_check = TRUE, full_check_report_level = level)
    full_spell_check_content <- readLines(file_path)
    expect_snapshot(full_spell_check_content)
    unlink(file_path)
  }

  # Test that the function returns the correct error when full_check_report_level is 
  # not specified while use_full_spell_check is TRUE
  expect_message(
    use_spell_check(use_full_spell_check = TRUE),
    "`full_check_report_level` is set to 'warning' by default"
  )
  unlink(file_path)

  # Test that the function returns an error when use_full_spell_check is FALSE and
  # full_check_report_level is set to "error"
  expect_error(
    use_spell_check(use_full_spell_check = FALSE, full_check_report_level = "error")
  )
  unlink(file_path)
})
